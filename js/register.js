$(function(){
    ...
    window.onload=function(){ 
        ...
        var cname1,cname2,cpwd1,cpwd2,cphone;
        function chkreg(){
            if((cname1=='yes')&&(cname2=='yes')&&(cpwd1=='yes')&(cpwd2=='yes')&&cphone=='yes'){//全部满足要求即可开启注册按钮
        ...
        $(".name").keyup(function(){   
            ...
            var verif_1=regname.match(/^[a-zA-Z_]/);
            if(verif_1==null){
                $("form").find("._tips").eq(0).html("必须以字母或下划线开头");
                cname1='';
            }
            else if(verif_1!=null&&regname.length<=3){
                $("form").find("._tips").eq(0).html("注册名称必须大于3位");
                cname1='';
            }
            else{
                $("form").find("._tips").eq(0).html('<span style="color:#00FF00">注册名称符合标准</span>');
                cname1='yes';
            }
            chkreg();//检查是否达到激活注册按钮标准
       });
        /*当用户名失去焦点时，系统调用函数检查用户名是否重复*/
        $(".name").blur(function(){ 
            var regname=$(".name").val(); 
            if(cname1=='yes'){
                var para='name='+regname;
                var url="chkname.php";
                var state_Change=stateChange_1;
